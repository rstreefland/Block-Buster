...
/**
 * <h1>Multiplayer.java</h1>
 * <p>
 * This class is responsible for running the multiplayer game mode. It generates
 * blocks randomly if no level file is supplied. It draws the bitmaps on the
 * screen and performs the game logic. It is derived from the SinglePlayer class
 * but has a number of key differences. There are two paddles, each of which
 * responds to separate onTouch events. The balls are also able to travel
 * through the top of the screen which is protected by the second paddle.
 * 
 * @author Rhys Streefland
 * @version 1.0
 * @since 2015-03-17
 * @see SinglePlayer GameView GameThread
 */
public class Multiplayer extends GameThread {
	...
	public Multiplayer(GameView gameView) {
		// House keeping
		super(gameView);
	...
	public void setupBeginning(Context mContext) {
		...
		// create two paddles and set their properties
		mPaddles = new Paddle[2];
		mPaddles[0] = new Paddle();
		mPaddles[0].mPaddleX = mCanvasWidth / 2;
		mPaddles[0].mPaddleY = mCanvasHeight / 17;
		mPaddles[0].mPaddleSpeedX = 0;
		mPaddles[1] = new Paddle();
		mPaddles[1].mPaddleX = mCanvasWidth / 2;
		mPaddles[1].mPaddleY = mCanvasHeight - mCanvasHeight / 17;
		mPaddles[1].mPaddleSpeedX = 0;
	...
	public void randomLevel() {
		...
		for (int i = 0; i < mNumberOfBlocks; i++) {
			...
			switch (blockType) {
			case 1:
				mBlocks[i] = new BlockTnt(blockX, blockY, mCellWidth, mCellHeight);
				break;
			case 2:
				mBlocks[i] = new BlockIce(blockX, blockY, mCellWidth, mCellHeight);
				break;
			case 3:
				...
				break;
			case 4:
				mBlocks[i] = new BlockWood(blockX, blockY, mCellWidth, mCellHeight);
				break;
			case 5:
				...
				break;
			case 6:
				mBlocks[i] = new BlockLava(blockX, blockY, mCellWidth, mCellHeight);
				break;
			case 7:
			...
			for (int i = 0; i < mNumberOfBlocks; i++) {
				...
				case "tnt":
					mBlocks[i] = new BlockTnt(Integer.parseInt(attributes[1]),
							Integer.parseInt(attributes[2]) + 2, mCellWidth,
							mCellHeight);
					break;
				case "ice":
					...
					break;
				case "stone":
					mBlocks[i] = new BlockStone(Integer.parseInt(attributes[1]),
							Integer.parseInt(attributes[2]) + 2, mCellWidth,
							mCellHeight);
					break;
				case "wood":
	...
	protected void doDraw(Canvas canvas) {
		...
		// draw each paddle
		canvas.drawBitmap(mImages.mPaddle2, mPaddles[0].mPaddleX
				- mImages.paddleWidth / 2, mPaddles[0].mPaddleY
				- mImages.paddleHeight / 2, null);
		canvas.drawBitmap(mImages.mPaddle, mPaddles[1].mPaddleX
				- mImages.paddleWidth / 2, mPaddles[1].mPaddleY
				- mImages.paddleHeight / 2, null);
			...
			if (block.isActive == true) {
				...
				case "craftingtable":
					canvas.drawBitmap(mImages.blockCraftingTable,
							block.mBlockX - mImages.blockWidth / 2,
							block.mBlockY - mImages.blockHeight / 2, null);
					break;
				default:
					break;
				}
	...
	// first set of coordinates for the first paddle
	@Override
	protected void actionOnTouch(float x, float y) {

		// set paddle to touch location
		mPaddles[0].mPaddleX = x;

		// keep paddle from going off screen - left
		if (mPaddles[0].mPaddleX < mImages.paddleWidth / 2)
			mPaddles[0].mPaddleX = mImages.paddleWidth / 2;

		// keep paddle from going off screen - right
		if (mPaddles[0].mPaddleX > mCanvasWidth - mImages.paddleWidth / 2)
			mPaddles[0].mPaddleX = mCanvasWidth - mImages.paddleWidth / 2;
	}

	// second set of coordinates for the second paddle
	@Override
	protected void actionOnSecondTouch(float x, float y) {

		// set paddle to touch location
		mPaddles[1].mPaddleX = x;

		// keep paddle from going off screen - left
		if (mPaddles[1].mPaddleX < mImages.paddleWidth / 2)
			mPaddles[1].mPaddleX = mImages.paddleWidth / 2;

		// keep paddle from going off screen - right
		if (mPaddles[1].mPaddleX > mCanvasWidth - mImages.paddleWidth / 2)
			mPaddles[1].mPaddleX = mCanvasWidth - mImages.paddleWidth / 2;
	}
	...
	protected void updateGame(float secondsElapsed) {
		...
		// If the ball moves down on the screen
			if (ball.isActive == true) {
				// Check for a paddle collision for each paddle
				detectCollision(ball, false, mPaddles[0].mPaddleX, 0,
						mMinDistanceBetweenBallAndPaddle);
				detectCollision(ball, false, mPaddles[1].mPaddleX,
						mCanvasHeight, mMinDistanceBetweenBallAndPaddle);

				// Move the ball's X and Y using the speed (pixel/sec)
				ball.mBallX = ball.mBallX + secondsElapsed
						* ball.mBallSpeedX;
				ball.mBallY = ball.mBallY + secondsElapsed
						* ball.mBallSpeedY;
				...
				if ((ball.mBallX <= mImages.ballWidth / 2 && ball.mBallSpeedX < 0)
						|| (ball.mBallX >= mCanvasWidth - mImages.ballWidth
								/ 2 && ball.mBallSpeedX > 0)) {
				...
				for (Block block : mBlocks) {
					...
					if (block.isActive == true) {
						if (detectCollision(ball, true, block.mBlockX,
								block.mBlockY,
								mMinDistanceBetweenBallAndblock)) {
				...
				// If all of the balls go out of the bottom or the top of the
				// screen lose a life
				// until the number of lives reaches zero
				if (ball.mBallY >= mCanvasHeight || ball.mBallY <= 0) {
					if (mActiveBalls > 1) {
						mActiveBalls--;
						ball.isActive = false;
					} else if (mLives > 1) {

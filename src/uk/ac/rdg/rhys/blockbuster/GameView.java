...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
	...
	public GameView(Context context, AttributeSet attrs) {
		...
		mHandler = new Handler() {
			...
			public void handleMessage(Message m) {
				...
				// if the message is a score
				if (m.getData().getBoolean("score")) {
					mScore  = m.getData().getString("text");
					MainActivity.sActionBar.setTitle("Score: "
							+ mScore + "    Lives: " + mLives);
				
				// if the message is lives
				} else if (m.getData().getBoolean("lives")) {
					mLives  = m.getData().getString("text");
					MainActivity.sActionBar.setTitle("Score: "
							+ mScore + "    Lives: " + mLives);
				} else {
					// So it is a status
					if (m.getData().getString("text").equals("lose")) {
						lose();
					} else if (m.getData().getString("text").equals("win")) {
						win();
					}
				}
			}
	...
	public void lose() {
		...
		builder.setPositiveButton(R.string.dialog_lose_button,
	...
	public void win() {
		...
		builder.setPositiveButton(R.string.dialog_win_button,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
						thread.doStart();
					}
		...
		// 2. Chain together various setter methods to set the dialog
		// characteristics
		builder.setMessage(R.string.dialog_win_message)
		...
		if (thread != null) {
			...
			if (thread.getState() == Thread.State.NEW) {
				...
				if (thread.getState() == Thread.State.TERMINATED) {
					// Start a new thread
					// Should be this to update screen with old game: new
					// GameThread(this, thread);
					// The method should set all fields in new thread to the
					// value of old thread's fields

					switch (currentView) {
					case VIEW_SINGLEPLAYER:
						thread = new SinglePlayer(this);
						break;
					case VIEW_MULTIPLAYER:
						thread = new Multiplayer(this);
						break;
					case VIEW_LEVELBUILDER:
						thread = new LevelCrafter(this);
						break;
					default:
						break;
					}
